...
- **Navigation Buttons:**
  - **Dimensions:** Fixed `44x44px` (width and height) for consistent touch target.
  - **Layout:** `display: flex`, `align-items: center`, `justify-content: center` for icon centering.
  - **Styling:**
    - Background: `none` (transparent).
    - Border: `none`.
    - Padding: `0` (no extra padding, size controlled by width/height).
    - Color: `white` (inherits header text color).
  - **Behavior:**
    - Use `flex-shrink: 0` to prevent shrinking in flexbox.
    - No `:hover` states; use `:active` with `opacity: 0.7` for tactile feedback.
    - Transition: `opacity 0.2s` for smooth feedback.
  - **Menu Button (`.btn-menu`):**
    - Font size: `1.5rem` (hamburger icon `☰`).
    - Used on root screens (home page).
  - **Back Button (`.btn-back`):**
    - Font size: `1rem`, font-weight: `600`.
    - Use a single `←` character (left arrow) for consistency.
    - Used on sub-pages (game, history detail, settings, etc.).
...
- **Format:** `v{version}-b{build}` where:
...
- **Fallback:** Display `v99.99.99-b99` when running in local development (where `@capacitor/app` returns empty values).
- **Container:**
...
5. **Splash Screen:** Use `@capacitor/splash-screen` to manually hide the splash screen (`launchAutoHide: false` in config) only after the React app has fully mounted, preventing a white flash. For Android, avoid stretched images by using an XML `layer-list` with a centered `<bitmap>` over a `<shape>` gradient.
   ...
   **Example `capacitor.config.ts`**:
   ...
   plugins: {
     SplashScreen: {
       ...
       launchAutoHide: false, // Wait for React
       backgroundColor: "#667eea",
     },
   }
   ```

   **Example `App.tsx` (Hide manually)**:
   ```typescript
   ...
   **Example `android/app/src/main/res/drawable/splash_background.xml` (Fix stretching)**:
   ```xml
